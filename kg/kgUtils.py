...
class KG:
    ...
    def clearGraph(self):
        self.graph.delete_all()  # 清除neo4j中原有的结点等所有信息

    def createKG(self, tripleList, persons):
        nodeDict = {}
        for item in tripleList:
            ...
            if item[2] not in nodeDict.keys():
    ...
    def delRela(self,rela):
        kg.graph.separate(rela)

    def delProperty(self,item,name):
        if item[name] != None:
            del item[name]
    ...
    def judgeSame(self,old,new,isNode=True):
        ...
        for key in prop1.keys():
            if prop1[key] != prop2[key] and prop2[key] != '':
                if prop1[key] != '':
                    if isNode:
                        situation = 0
                    else:
    ...
    def addNode(self,label,prop):
        ...
        if label == '人物':
            ...
            for node in res:
                situation,data = self.judgeSame(node,newNode)
                if situation != 0:
                    oldNode = node
                    break
        elif label == '组织机构':
